    ...
    if (car.images && car.images.length > 0) {
        galleryHtml = `
            <div class="car-gallery">
                ...
                <div class="thumbnails-container" style="display: flex; gap: 10px; margin-top: 10px; overflow-x: auto; padding-bottom: 5px;">
        `;
        
        car.images.forEach((image, index) => {
            galleryHtml += `
                <img src="${image}" alt="${car.make} ${car.model}" class="thumbnail" data-index="${index}" style="width: 70px; height: 50px; object-fit: cover; cursor: pointer; border-radius: 4px; opacity: ${index === 0 ? '1' : '0.6'}; transition: opacity 0.2s;">
            `;
        });
